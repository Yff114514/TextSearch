...
public class MinHash implements Serializable {
     ...
     * @param dict_size
     */
    public MinHash(final int size, final int dict_size) throws FileNotFoundException {
     ...
     * @param dict_size
     */
    public MinHash(final double error, final int dict_size) throws FileNotFoundException {
     ...
     */
    public MinHash(final int size, final int dict_size, final long seed) throws FileNotFoundException {
     ...
     *                  guarantee identical hashes across object instantiations
     */
    public MinHash(final double error, final int dict_size, final long seed) throws FileNotFoundException {
     ...
     * @param r
     */
    private void init(final int size, final int dict_size, final Random r) throws FileNotFoundException {
        if (size <= 0) {
        ...
        this.n = size;
        hash_coefs = new long[n][5];

        File hashCoefsFile = new File("./src/data/hash_coefs.txt");
        FileInputStream fileInputStream = new FileInputStream(hashCoefsFile);
        InputStreamReader inputStreamReader = new InputStreamReader(fileInputStream);
        BufferedReader bf = new BufferedReader(inputStreamReader);
        StringBuffer sb = new StringBuffer();
        try {
            if (hashCoefsFile.exists()) {

                //String lines = new String();
                for (int i = 0; i < n; i++) {
                    String[] lineArr =bf.readLine().split(" ");
                    hash_coefs[i][0] = Integer.parseInt(lineArr[0]);
                    hash_coefs[i][1] = Integer.parseInt(lineArr[1]);
                }
                return;
            }
            ...
            for (int i = 0; i < n; i++) {
                ...
                builder.append(hash_coefs[i][0]).append(" ").append(hash_coefs[i][1]).append("\n");
            }

            FileWriter fileWriter = new FileWriter(hashCoefsFile.getAbsoluteFile());
            BufferedWriter bw = new BufferedWriter(fileWriter);
            bw.write(builder.toString());
            bw.close();

            //FileUtils.write(hashCoefsFile, builder.toString());
        } catch (IOException e) {
            e.printStackTrace();
        }
//        System.out.print(hash_coefs.length + "---" + hash_coefs[0].length);
    }
